...
window.verDetalle = (id) => {
    ...
    // Verificar configuración de delivery
    ...
    document.getElementById('detail-price-bs').innerText = precios.localFormateado + ' ' + precios.moneda;
...
function renderInterface() {
    ...
    document.getElementById('shop-name').innerHTML = `${nombreNegocio} <span class="text-${tema.primary}">${acentoNegocio}</span>`;
    ...
    if (ivaNoteEl) {
        ...
        ivaNoteEl.innerText = configNegocio.exentoIVA ? '' : `Total en ${config.moneda} incluye IVA (${(config.ivaDefault * 100)}%)`;
    }

    // ========== CAMPO DE MESA (MEJORADO) ==========
    const mesaContainer = document.getElementById('mesa-container');
    if (mesaContainer) {
        // Tipos que NO deben mostrar mesa (solo ferreterías y tiendas)
...
function conectarFirestore() {
    ...
    unsubscribeFirestore = onSnapshot(docRef, (snap) => {
        ...
        document.getElementById('shop-name').innerHTML = `<span class="text-red-500 text-xs">Error de conexión</span>`;
    });
}
